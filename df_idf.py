...
stopwords_path="D:/jupyter_workfile/haizhi/nlp/data/stopwords.txt" #网上载的停用词表+ziji
...
#cropus=words_tr+words_val #+words_test
tfidf_vect = TfidfVectorizer(analyzer='word', token_pattern=r'\w{2,}',stop_words=stopwords,max_features=5000)
tfidf_vect.fit(cropus) #[cropus]
...
logis_clf=linear_model.LogisticRegression() #class_weight="balanced" 精度下降0.76->0.72
xgb_clf=xgboost.XGBClassifier()

auc_nb,accuracy_nb ,f1_nb,recall_nb,precision_nb, _ = train_model(nb_clf, xtrain_tfidf.toarray(), train_y, xvalid_tfidf.toarray(),valid_y)
print("NB, WordLevel TF-IDF ","auc:",auc_nb,"accuracy: ",accuracy_nb,"f1_score:",f1_nb,"recall_score:",recall_nb,"precision",precision_nb)
auc_lr,accuracy_lr ,f1_lr,recall_lr,precision_lr, _= train_model(logis_clf, xtrain_tfidf.toarray(), train_y, xvalid_tfidf.toarray(),valid_y)
print("LR, WordLevel TF-IDF: ","auc:",auc_lr,"accuracy: ",accuracy_lr,"f1_score:",f1_lr,"recall_score:",recall_lr,"precision",precision_lr)
auc_xgb,accuracy_xgb ,f1_xgb,recall_xgb,precision_xgb, _= train_model(xgb_clf, xtrain_tfidf.tocsc(), train_y, xvalid_tfidf.tocsc(),valid_y)
print ("Xgb, WordLevel TF-IDF: ","auc:",auc_xgb,"accuracy: ",accuracy_xgb,"f1_score:",f1_xgb,"recall_score:",recall_xgb,"precision",precision_xgb)


#检验
# _,_ ,_ ,_ ,pred = train_model(xgb_clf, xtrain_tfidf.toarray(), train_y, xvalid_tfidf.toarray())
# pred=pd.Series(pred)
...
# result.to_csv('D:/jupyter_workfile/haizhi/nlp/result/modify_xgb_pred_valid.csv',encoding="utf_8_sig")  #utf_8_sig是为了解决乱码问题，utf_8也是乱码

#预测
# _,_ ,_ ,_ ,pred = train_model(logis_clf, xtrain_tfidf.toarray(), train_y, xtest_tfidf.toarray())
# pred=pd.Series(pred)
# print(pred.value_counts())  #1-36928 0-2403检验结果有偏差
# result=pd.DataFrame(list(zip(test_x,pred)),columns=['text','pred_label'],)
# result.to_csv('pred_modify_unlabeled.csv',encoding="utf_8_sig")  #utf_8_sig是为了解决乱码问题，utf_8也是乱码